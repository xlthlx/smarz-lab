...
function smarz_remove_pings() {
	...
	add_action( 'template_redirect', 'smarz_remove_xml_rpc_tag_buffer_start',
		- 1 );
...
function smarz_wp_template_loader() {
	...
	if ( $_SERVER['REQUEST_URI'] === smarz_user_trailingslashit( str_repeat( '-/',
			10 ) ) ) {
...
function smarz_new_login_url( $scheme = null ) {
	...
	if ( get_option( 'permalink_structure' ) ) {
		return smarz_user_trailingslashit( home_url( '/',
				$scheme ) . $smarz_login['smarz_login'] );
	}
